...
---@type ACTIONS.hide_last
local function hide_last(FLOAT, opts)
    -- setup
    ...
    if opts.position_list == nil and opts.style == nil then
        ...
        FLOAT.actions.hide(FLOAT, { bufwin_state = last })
    ...
    while pos_index >= 1 do
        ...
        while window_index >= 1 do
            ...
            if win.position == pos then
                FLOAT.actions.hide(FLOAT, { bufwin_state = win })
                return true
            end
            ::contiue_win::
